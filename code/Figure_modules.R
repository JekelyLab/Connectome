#R code to generate the figures showing the different Leiden modules of the connectome graph
...
#library(leiden)
#load rjson to read json files
library(rjson)
library(data.table)

source("~/R/conn.R")
setwd("/Users/gj274/OneDrive\ -\ University\ of\ Exeter/Paper/Connectome/Figures/Figure_modules/")

#graph_json <- fromJSON(file = "graph1.json")
...
#import from catmaid json
#for (i in c(1:length(graph_json$elements$nodes))){
#  cells_with_color[i,1] <- graph_json$elements$nodes[[i]]$data$id
#  cells_with_color[i,2] <- graph_json$elements$nodes[[i]]$data$color
#}
...
for (i in c(1:nrow(partitions))){
  ...
  rgl.snapshot(paste("connectome_cluster", i, ".png",sep="") )
  clear3d()
}
